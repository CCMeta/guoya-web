	...
	<view class="container position-relative">
		<view style="margin-bottom: 130rpx;">
			<view class="section-1">
				...
				<template v-if="orderType == 'takein'">
					<list-cell class="contact" :hover="false">
						<view class="flex-fill d-flex justify-content-between align-items-center">
							<view class="title ">收货人</view>
							<view class="time">
								<input class="text-right" placeholder="请输入收货人" v-model="form.name"/>
							</view>
						</view>
					</list-cell>
					<list-cell class="contact" :hover="false">
					...
					<list-cell class="contact" :hover="false">
						<view class="flex-fill d-flex justify-content-between align-items-center">
							...
							<view class="time flex-fill">
								<textarea style="width: 100%;padding-left: 20rpx;" placeholder="在此处粘贴收件信息,程序可自动识别内容并填写, 包括姓名,电话,地址" v-model="form.address"/>
							</view>
						</view>
					</list-cell>
					<list-cell class="contact" :hover="false">
						<button style="width: 100%;" type="primary" plain="true" @click="goToResolve">
							智能识别信息
						</button>
					</list-cell>
				</template>
			...
			<view class="section-2">
				<view class="cart d-flex flex-column">
					<list-cell v-for="(item, index) in cart" :key="index">
						<view class="w-100 d-flex flex-column">
			...
			<!-- 支付方式 end -->
			<!-- 备注 begin -->
			<list-cell arrow last @click="goToRemark" style="display: none;">
...
<script>
	...
	export default {
		...
		methods: {
			...
			goToResolve(){
				let contact = addressParse(this.form.address);
				if(contact.name && contact.phone){
					let address = contact.province + contact.city + contact.area + contact.detail
					this.$set(this.form, 'mobile', contact.phone)		
					this.$set(this.form, 'name', contact.name)
          ...
          if ( this.form.address.length && this.form.name.length && this.form.mobile.length ){
            this.pay()
          } else {
            uni.showToast({
            	title: '请检查地址，姓名，电话不可为空'
            })
          }
				}
			},
			async pay() {
				...
				//测试订单
				let order = this.cart
				let response = await this.$fuck('orders/post', { 'info':this.form, 'goods':order }, 'POST')
