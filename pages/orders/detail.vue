<template>
	<view class="container" style="padding:20rpx;">
					...
					<list-cell :hover="false">
						<view class="w-100 d-flex align-items-center">
							<view class="d-flex flex-column w-60">
								<view class="w-100 font-size-lg text-color-base text-truncate mb-10">{{ order.name }} ({{order.mobile}})</view>
								<view class="w-100 d-flex align-items-center">
									<view class="font-size-sm text-color-assist">{{ order.address }}</view>
								</view>
							</view>
						...
						<view class="w-100 d-flex flex-column position-relative" style="margin-bottom: -40rpx;">
							<view class="w-100 d-flex align-items-center mb-40" v-for="(good, index) in order.goods" :key="index">
								<view class="d-flex flex-column w-60 overflow-hidden">
									<view class="font-size-lg text-color-base mb-10 text-truncate">{{ good.good_name }}</view>
								...
								<view class="d-flex w-40 align-items-center justify-content-between pl-30">
									<view class="font-size-base text-color-base">x{{ good.good_count }}</view>
									<view class="font-size-base text-color-base font-weight-bold">￥{{ good.good_price }}</view>
								</view>
							</view>
						</view>
					</list-cell>
				...
				<view class="section">
					...
					<list-cell :hover="false" padding="50rpx 30rpx">
						<view class="w-100 d-flex flex-column">
							...
							<view class="pay-cell">
								<view>支付方式</view>
								<view class="font-weight-bold">{{ order.pay_mode || "微信支付" }}</view>
							</view>
							<view class="pay-cell">
								<view>订单号</view>
		...
		<view class="btn-box">
			<view class="item">
				<button type="primary">复制收货信息</button>
			</view>
		</view>
	</view>
</template>
